...
|  測試類型  | 定義                            | 測試內容         | 測試目的       |
|:----------:| ------------------------------- | ---------------- | -------------- |
|   α測試    | 在內部測試人員中進行的初步測試  | 核心功能         | 發現主要缺陷   |
|   β測試    | 在真實環境下,由使用者進行的測試 | 工作流和使用場景 | 改進使用者體驗 |
| 相容性測試 | 測試軟件和硬體的相容性          | 不同環境組態     | 確保相容性     |
|   UI測試   | 測試使用者介面                  | 介面和互動       | 提高易用性     |
|  冒煙測試  | 選擇核心功能快速測試            | 關鍵功能         | 驗證基本可用性 |
|  功能測試  | 測試各個功能的正確性            | 業務功能         | 發現功能缺陷   |
|  系統測試  | 在真實環境對整體系統進行測試    | 性能,安全性等    | 驗證系統穩定性 |
## α測試 ( Alpha Testing )
...
## 系統測試 ( System Testing )
### 定義
- 在真實環境下對系統整體進行測試,確保各模塊能正常協作的測試活動。
### 測試內容
- 軟硬體功能、性能、安全性、可靠性、可用性等。
### 常用測試方法
- 整體回歸測試、壓力測試、容量測試等。
### 測試目的
- 確保系統按需求設計運行,可靠及穩定。
